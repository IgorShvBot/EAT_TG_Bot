...
class Database:
    ...
    def save_transactions(self, df, user_id):
        ...
        try:
            with self.get_cursor() as cur:
                ...
                df['сумма'] = df['сумма'].astype(str).str.replace(',', '.', regex=False)
                df['сумма'] = pd.to_numeric(df['сумма'], errors='coerce')
                if 'сумма (куда)' in df.columns:
                    df['сумма (куда)'] = df['сумма (куда)'].astype(str).str.replace(',', '.', regex=False)
    ...
    def get_last_import_ids(self, user_id: int, limit: int = 10) -> list[tuple[int, datetime]]:
            ...
            query = """
                SELECT DISTINCT ON (import_id) import_id, created_at
                FROM transactions
                WHERE user_id = %s
