...
### Изменено
- Команда `/edit`: выбор из сохранённых шаблонов `/edit_templates`
...
### fix
- Кнопка (Создать из ID) для сохранение шаблона из `/edit_templates` вместо `/edit` - кнопка Сохранить шаблон
(решена ошибка после ввода "Введите название шаблона:")


------------------------------------------------------------------------


## [v3.11.2(1)] - 2025-07-09
### Добавлено
- Кнопка (Создать из ID) для сохранение шаблона из `/edit_templates` вместо `/edit` - кнопка Сохранить шаблон
...
### fix
- Исправлена (полностью) ошибка сохранения шаблона Команды `/edit` (Команда `/edit_tamlates`)
(осталась ошибка после ввода "Введите название шаблона:")
...
## [v3.11.2] - 2025-07-09
### Добавлено
- Кнопка (Создать из ID) для сохранение шаблона из edit_templates вместо `/edit` - кнопка Сохранить шаблон (пока не работает)
...
### Изменено
- Команда `/edit`: Добавлена возможность редактирования кнопок
- Команда `/export`: Добавлена возможность выгрузки по полю ID

### fix
- Исправлена (не до конца) ошибка сохранения шаблона Команды `/edit` (Команда `/edit_tamlates`)
...
## [v3.11.0] - 2025-07-07
### Добавлено
- Команда `/edit_tamlates`: Добавлен параметр для подстановки данных из БД и управление шаблонами редактирования

### Изменено
- Команда `/edit`: Добавлен текст пользователя в кнопки фильтров (edit.py)
- Команда `/edit`: Добавлен параметр (ID записи) для подстановки данных из БД
...
## [v3.10.0] - 2025-06-27
### Добавлено
- Добавлена команда для создания бэкапа БД (bot.py - def create_db_backup)
- Команда `/export`: добавлено изменение фильтра даты (старта и окончания) на +- месяц для фильтра кнопками (handlers/export)

### Изменено
- Команда `/edit`: возможность редактировать поле «Наличность (куда)», "Тип транзакции" и "Класс"
- Команда `/edit`: для выбора нескольких параметров редактирования одновременно
- Команда `/export`: "Дата окончания" = последний день месяца

### fix
- команда /config, редактировать Конфиг, при выборе Категории или Спец.условия не получаем ошибку (handlers/config_handlers.py)
- Команда /export, исправлена работа фильтра по описанию
- Переместил команду add_pattern в подменю
...
### fix
- команда /export, то в фильтре "ID импорта" теперь подтягивается дата и время с учетом часового пояса +3 (изменения в get_last_import_ids)
- при сохранении в БД ID записям присваиваются в хронологическом порядке относительно даты и времени операций (добавлена сортировка в save_transactions)
...
## [v3.9.0] - 2025-06-17
### Добавлено
- Добавлена кнопка просмотра найденных дубликатов перед их обновлением

### Изменено
- Оптимизировано меню управления настройками - меню `/settings`
- Меню управления конфигурацией (в нем можно добавить паттерн) - /config

### fix
- команда /config, не пропадает меню выбора "Выберите конфигурационный файл:" после выбора файла
