...
- Команда `/edit_templates` для шаблонов редактирования (новое в v3.11.0)
- После формирования отчёта доступна кнопка «Сохранить шаблон» (v3.8.0)
- Создание резервной копии базы данных командой `/backup` (новое в v3.10.0)
- В меню `/export` можно указать ID записи или диапазон для выгрузки (новое в v3.11.1)
...
### Управление конфигурацией
/config - меню управления конфигурацией (здесь можно добавить паттерны)
/add_pattern "Категория" "Паттерн" - добавить паттерн к категории (команда вызывается из меню /config)
Кнопка «Добавить Категорию - Паттерн» открывает интерактивный выбор категории.

### Резервное копирование базы
/backup - создать бэкап БД и получить файл
...
- Для управления настройками используйте меню `/settings`
